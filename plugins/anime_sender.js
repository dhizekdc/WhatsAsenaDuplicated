...
Asena.addCommand({pattern: 'animepic ?(.*)', fromMe: wk, desc: pic, usage: 'animepic normal // animepic nsfw'}, (async (message, match) => {
  if (match[1] == 'normal') {
    ...
    await message.sendMessage(Buffer.from(image_buffer.data), MessageType.image, { mimetype: Mimetype.png, caption: 'Made by WhatsDhizek' })
    ...
    await message.sendMessage(Buffer.from(image_buffer_nsfw.data), MessageType.image, { mimetype: Mimetype.png, caption: 'Made by WhatsDhizek' })
  } else {
...
Asena.addCommand({pattern: 'animegif ?(.*)', fromMe: wk, desc: giff, usage: 'animegif normal // animegif nsfw'}, (async (message, match) => {
  if (match[1] == 'normal') {
    ...
    await download(gif_link, '/root/WhatsAsenaDuplicated/pic.mp4', async() => {                          
      await message.client.sendMessage(message.jid, fs.readFileSync('/root/WhatsAsenaDuplicated/pic.mp4'), MessageType.video, { caption: 'Made by WhatsDhizek', mimetype: Mimetype.gif })
    })
  } else if (match[1] == 'nsfw') {
    ...
    await download(gif_link_nsfw, '/root/WhatsAsenaDuplicated/picn.mp4', async() => {                          
      await message.client.sendMessage(message.jid, fs.readFileSync('/root/WhatsAsenaDuplicated/picn.mp4'), MessageType.video, { caption: 'Made by WhatsDhizek', mimetype: Mimetype.gif })
    })
  } else {
    return await message.client.sendMessage(message.jid,wr_usage,MessageType.text)
  }
